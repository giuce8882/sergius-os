...
// --- SECURITY BRIEF ---
// DATA SENT: The prompt includes the user's chat input, the name of the 'focused task' (if any), 
// and a sanitized list of current active todo items.
// SKILLS: App features local-only endpoints for FS access. Review-Driven Workflow ensures user approval first.
// ----------------------
...
const BrainstormChat = ({ isOpen, onClose, taskTarget, todos, setTodos, extractProjectIdAsync, activeProjectId }) => {
    ...
    const fetchLogs = async () => {
        ...
        try {
            // Phase 6: Vault Maintenance - Auto-archive old logs before fetching
            await fetch('/api/fs/archive-logs', {
                method: 'POST',
                body: JSON.stringify({ projectId: activeProjectId })
            });

            // Fetch remaining active logs
    ...
    const handleSend = async (e) => {
        ...
        try {
            ...
            const historyData = messages.slice(1).map(m => ({
            ...
            if (functionCalls && functionCalls.length > 0) {
                setPendingArtifact(functionCalls[0]);
                setIsLoading(false);
                return; // Pause for user approval
            }

            processTextResponse(result.response.text());
    ...
    const handleApproveArtifact = async () => {
        ...
        try {
            if (call.name === 'create_folder') {
                ...
                // Internal React App Skill
                const newTasks = call.args.tasks.map((taskSpec, index) => {
                    const newId = Date.now() + index;
                    // Kickoff background extraction for color hashing (or pass the ID if standard)
                    ...
                    return {
                        ...
                        projectColor: null // Will be instantly updated by the async extractor
                    };
                });

                // Inject directly into React state
                ...
                setIsLoading(false);
                return; // UI triggered this, not Gemini. Stop here.
            }
        } catch (e) {
    ...
    const handleDenyArtifact = async () => {
        ...
        if (call.name === 'save_chat_log') {
            // UI triggered, don't ping Gemini
            return;
        }
        ...
        try {
            const result = await chatSessionRef.current.sendMessage([{
                functionResponse: { name: call.name, response: { error: "User denied permission to execute this skill." } }
            }]);
            processTextResponse(result.response.text());
        } catch (e) {
            setIsLoading(false);
                        ...
                        {savedLogs.length === 0 ? (
                            ...
                            savedLogs.map((log, idx) => (
                                <div key={idx} className="bg-black/20 border border-white/5 rounded-2xl p-4 hover:bg-white/5 transition-colors group cursor-pointer" onClick={() => {
                                    // PREVIEW MODE: We just dump the transcript into an alert or a distinct view. For now, since "Preview Mode" was requested so it doesn't overwrite the active chat, we will just alert it or temporarily swap messages without saving state. A better approach is rendering a local accordion.
                                ...
                                <div key={idx} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                                    <div
                                        className={`max-w-[85%] rounded-2xl p-4 ${msg.role === 'user'
                                            ? 'bg-blue-500/30 border border-blue-400/20 rounded-tr-sm backdrop-blur-md'
                                ...
                                <div className="flex justify-start">
                                    <div className="bg-orange-500/10 border border-orange-500/30 rounded-2xl rounded-tl-sm p-4 backdrop-blur-md max-w-[90%]">
                                        ...
                                        <div className="flex gap-2">
                                            <button onClick={handleApproveArtifact} className="flex-1 bg-orange-500 hover:bg-orange-600 text-white py-1.5 rounded-lg text-sm font-medium transition-colors">
                                                Approve
                                            </button>
                                            <button onClick={handleDenyArtifact} className="flex-1 bg-white/10 hover:bg-white/20 text-white/80 py-1.5 rounded-lg text-sm font-medium transition-colors">
                                                Deny
                                            </button>
                                        </div>
                                    </div>
